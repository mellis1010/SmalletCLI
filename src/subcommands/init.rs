...
pub fn process(path: &Path) -> Result<()> {
    ...
    let cfg = if let Some(cfg) = maybe_cfg {
        println!("Cosmic.toml already exists in workspace");
        cfg.into_inner()
    } else {
        let cfg = Config::default();
        let mut file = File::create("Cosmic.toml")?;
    ...
    for (cluster, _key) in result.iter() {
        if cluster.clone() != Cluster::Mainnet {
            let path_string = format!(".cosmic/deployers/{}.json", cluster);
    ...
    println!("Cosmic initialized! Please add the .cosmic/ directory to your gitignore.");
